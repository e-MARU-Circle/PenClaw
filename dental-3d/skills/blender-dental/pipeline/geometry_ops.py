...
_JP_FONTS = ("Hiragino Sans", "Hiragino Kaku Gothic ProN", "Hiragino Maru Gothic ProN",
             "Noto Sans CJK JP", "Noto Sans JP", "Yu Gothic",
             "IPAexGothic", "IPAGothic", "TakaoGothic")


def _covers(font_path: str, text: str) -> bool:
    """フォントが text 全文字のグリフを実際に持つか（豆腐□がそのまま彫られる事故の防止）。"""
    from matplotlib import ft2font
    f = ft2font.FT2Font(str(font_path))
    try:
        f.select_charmap(0x756E6963)  # FT_ENCODING_UNICODE（既定cmapが非Unicodeの稀なフォント対策）
    except Exception:
        pass
    return all(f.get_char_index(ord(c)) != 0 for c in text if not c.isspace())


def _resolve_font(text: str, font: str | None = None):
    """刻印フォントを解決する。font=TTF/TTC/OTFパス or ファミリー名（グリフ不足なら明示エラー）。
    未指定なら DejaVu Sans→（グリフ不足時）日本語フォント候補の順で自動検出（日本語刻印対応）。"""
    import os
    ...
    if font:
        if os.path.exists(str(font)):
            ...
            try:  # フォールバック禁止: タイポ時に黙ってDejaVuで彫られる事故を防ぐ
                path = findfont(FontProperties(family=str(font)), fallback_to_default=False)
            except Exception as e:
                raise ValueError(f"指定フォント『{font}』が見つかりません（名前を確認するかファイルパスで指定）") from e
        if not _covers(path, text):
            raise ValueError(f"指定フォント『{font}』に『{text}』のグリフがありません: {path}")
        return FontProperties(fname=path, weight="bold")
    path = findfont(FontProperties(family="DejaVu Sans"))
    if _covers(path, text):
        return FontProperties(family="DejaVu Sans", weight="bold")
    for fam in _JP_FONTS:
        try:  # weight指定で太ウェイトのファイル自体を選ぶ（fname指定はweightが無視されるため）
            p = findfont(FontProperties(family=fam, weight="bold"), fallback_to_default=False)
        except Exception:
            continue
        if p and _covers(p, text):
            return FontProperties(fname=p, weight="bold")
    raise RuntimeError(f"『{text}』を刻印できるフォントが見つかりません。"
                       "--engrave-font でフォント名かTTF/TTC/OTFのパスを指定してください")


def _text_polygons(text: str, size_mm: float, font: str | None = None):
    """テキストを2D shapelyポリゴン群へ（穴つき字形A/D/0や漢字の閉領域はeven-odd規則で処理）。
    依存: matplotlib（刻印機能使用時のみ必要）。日本語はフォント自動検出（_resolve_font）。"""
    try:
    ...
    from shapely.ops import unary_union
    import unicodedata
    text = unicodedata.normalize("NFC", str(text))   # NFD結合文字（濁点分解等）の位置ズレ防止
    tp = TextPath((0, 0), text, size=float(size_mm),
                  prop=_resolve_font(text, font))
    ...
    # 最小ストローク幅ガード: 0.2mmの開処理で面積が半減するなら線が細すぎ＝印刷で潰れる恐れ
    # （較正: IPAexゴシックのストローク≈文字高の7%。文字高3.0mm=0.2mm=レジンプリンタXY50µmの実用下限）
    thin = merged.buffer(-0.10).buffer(0.10)
    if thin.is_empty or thin.area < merged.area * 0.5:
        print(f"警告: 文字高{float(size_mm):.1f}mmでは刻印の線幅が0.2mm未満に潰れる恐れ。"
              "文字高(--engrave-height)の引き上げ・凸(--engrave-emboss)・文字数削減を推奨。")
    return merged
...
def engrave_case_code(model: trimesh.Trimesh, text: str, bd, *,
                      ...
                      azimuth_deg: float = 0.0, font: str | None = None):
    ...
    font はフォント名/パス（未指定なら自動検出。日本語「初診」等も可）。
    注意: 患者氏名は彫らない。カルテ番号・治療ステージ等の匿名相当ラベルのみ（D-032）。"""
    bd = np.asarray(bd, np.float64)
    bd = bd / (np.linalg.norm(bd) + 1e-9)
    V = np.asarray(model.vertices, np.float64)
    ...
    if height_mm is None:
        ...
        if any(ord(c) >= 0x3000 for c in str(text)):     # CJKはストローク潰れ防止で最小3mm
            height_mm = min(max(float(height_mm), 3.0), float(rim_mm))
    ...
    poly = _text_polygons(text, height_mm, font)
